// 圖片 manifest:把每頁對到 public/story/ 的圖檔。
// 目前無圖:src 全為 null → 由 StoryImage 渲染乾淨佔位(純色 + placeholderLabel)。
// 日後把圖放進 public/story/,再把對應 src 換成 "/story/檔名.png" 即自動取代佔位。
// TODO: replace ── 補圖後逐一填入 src(不要在此處猜檔名)。
